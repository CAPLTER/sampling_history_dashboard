"""
Shared code for building a sampling-history dashboard page from a site table.

This module knows nothing about where a dataset's raw files come from or how
its site table is assembled -- that's each dataset's own loader (see
arthropod_timeline/build_timeline.py and bird_timeline/build_birds.py). It
starts from a standard site table (one row per site: site_code, lat, long,
start_date, end_date, status, plus optionally a grouping column) and turns it
into NLCD-enriched data, a timeline, a map, and the CSV exports, all of which
are shared verbatim between datasets.
"""
...
# Pin the CDN bundle to the plotly.js that generated the figure JSON, so a
# page can never drift from the installed plotly version.
PLOTLYJS_VERSION = get_plotlyjs_version()

# NLCD snapshot years used by every dataset built from this repo.
NLCD_YEARS = [2001, 2006, 2011, 2016, 2019, 2021]

# A committed lookup of NLCD codes keyed by coordinate, shared by every
# dataset built from this repo. The MRLC service this queries is unreliable
# (observed outright ServiceUnavailableError and incomplete responses during
# development), so once a coordinate has been looked up it is never queried
# again: a build with nothing new to look up has no network dependency at
# all. Keyed by coordinate rather than site_code because the same site_code
# can denote a different physical location in a different dataset (confirmed
# true of the arthropod and bird site tables), so a site-keyed cache could
# silently mix them up.
NLCD_CACHE_PATH = Path(__file__).resolve().parent / "nlcd_lookup.json"
...
# ---------------------------------------------------------------------------
# Site-table helpers shared by every dataset's loader
# ---------------------------------------------------------------------------

def assign_study_end_status(
    df: pd.DataFrame,
    window: pd.Timedelta,
    to_study_end_label: str,
    retired_label: str,
) -> pd.DataFrame:
    """
    Label each site by whether it was sampled up to the end of the study.

    A site counts as having run to the end when its last sampling event falls
    within `window` of the most recent event anywhere in the data; otherwise
    it is labelled as retired early. Adds a `status` column in place and
    returns df for convenience.
    """
    study_end = df["end_date"].max()
    df["status"] = (df["end_date"] >= study_end - window).map(
        {True: to_study_end_label, False: retired_label}
    )
    return df


def warn_duplicate_coordinates(df: pd.DataFrame, precision: int = 6) -> None:
    """
    Log a warning if two different sites resolve to the same coordinate.

    Two sites sharing a point is usually a data error (or a resolution
    ambiguity that should have been broken some other way), so this is worth
    surfacing even though it doesn't stop the build.
    """
    valid = df.dropna(subset=["lat", "long"])
    keyed = valid.assign(
        _coord=list(
            zip(valid["lat"].round(precision), valid["long"].round(precision))
        )
    )
    for coord, group in keyed.groupby("_coord"):
        sites = sorted(group["site_code"])
        if len(sites) > 1:
            logging.warning(
                "Sites share the same coordinate %s: %s", coord, ", ".join(sites)
            )


# ---------------------------------------------------------------------------
# NLCD lookup, backed by a committed cache
# ---------------------------------------------------------------------------
...
def build_segmented_timeline_html(
    ...
    Sites stop at their actual end_date (creating a blank/gap after).

    If `group_by` names a column in df (e.g. "location_type"), sites are
    ordered by that column, a divider line is drawn at each group boundary
    instead of every 5 sites, and a second axis label marks each group.
    Passing None (the default) reproduces the ungrouped layout exactly.
    """
    rows = []
    group_of_site: dict[str, str] = {}
    ...
    for idx, row in df.iterrows():
        ...
        if group_by is not None:
            group_of_site[site] = row[group_by]
...
# Final segment after last snapshot
    ...
    if group_by is not None:
        # Order sites by group (alphabetically by group, then by site_code
        # within a group), so the axis reads as blocks rather than being
        # incidentally grouped by whatever order the input happened to have.
        unique_sites = sorted(
            seg_df["site_code"].unique(),
            key=lambda s: (group_of_site.get(s, ""), s),
        )
    else:
...
# add dotted lines for each NLCD snapshot year with individual labels
    ...
    if group_by is None:
        ...
        # One divider at each group boundary instead of every 5 sites, plus a
        # second axis label naming each group, vertically centred over its rows.
        left_margin = 200
        boundaries_idx = []
        for i, site in enumerate(unique_sites):
            if i > 0 and group_of_site.get(site) != group_of_site.get(unique_sites[i - 1]):
        ...
        for i in boundaries_idx:
            ...
            fig.add_shape(
                ...
                y0=y_position, y1=y_position, yref="paper",
                line=dict(width=1, color="rgba(150, 150, 150, 0.6)", dash="solid"),
                layer="below",
            )

        group_bounds = [0] + boundaries_idx + [num_sites]
        for start_i, end_i in zip(group_bounds[:-1], group_bounds[1:]):
            mid_site = unique_sites[(start_i + end_i - 1) // 2]
            y_mid = 1 - ((start_i + end_i) / 2) / num_sites
            group_annotations.append(
                dict(
                    ...
                    xref="paper",
                    yref="paper",
                    text=f"<b>{group_of_site.get(mid_site, '')}</b>",
                    showarrow=False,
                    textangle=-90,
                    font=dict(size=10, color="rgba(55,65,81,0.9)"),
                    xanchor="center",
                    yanchor="middle",
                )
            )
    ...
    fig.update_yaxes(
        ...
        categoryarray=unique_sites,  # All site names in order - this ensures all sites are categories
    ...
    if group_by is not None:
        caption += " Sites are grouped and separated by solid lines."

    fig.update_layout(
        legend_title="NLCD land-cover class",
        template="plotly_white",
        margin=dict(l=left_margin, r=40, t=100, b=50),  # left margin fits site labels (+ group labels)
        ...
        annotations=nlcd_annotations + group_annotations + [
            dict(
                text=caption,
                xref="paper",
                yref="paper",
...
def build_map_html(df: pd.DataFrame) -> tuple[str, dict[str, float] | None]:
    df = df.copy()
    df["lat"] = pd.to_numeric(df["lat"], errors="coerce")
    df["long"] = pd.to_numeric(df["long"], errors="coerce")
...
def export_enriched_data(
    ...
    try:
        ...
        raise RuntimeError(f"Could not create output directory {output_dir}") from exc
    ...
    # A build that cannot write its declared output is not a successful
    # build; a warning that only prints is easy to miss in CI, so this raises.
    try:
        summary_df.to_csv(summary_path, index=False, encoding="utf-8")
        detailed_df.to_csv(detailed_path, index=False, encoding="utf-8")
    except OSError as exc:
        raise RuntimeError(f"Could not write CSV output to {output_dir}") from exc
...
def build_full_page(
    ...
    *,
    page_title: str = "CAP LTER – Arthropod Timeline & Map",
    heading: str = "CAP LTER Arthropod Sites (Sampling and Land Use)",
    timeline_card_heading: str = "Sampling history by land-use change",
    timeline_card_caption: str = "Arthropod Sampling Sites (Timeline by land-use change)",
    map_card_heading: str = "Site locations by land-use context",
    map_card_caption: str = "Arthropod Sampling Sites (Land-Use context with latest NLCD)",
    map_note: str = (
    ...
    """HTML wrapper with timeline above map. Every text field is overridable
    so the same page shell serves every dataset; the defaults reproduce the
    original arthropod-only page."""
    ...
    # Attached directly to the following line rather than kept as its own
    # line, so an empty nav_html (the default) reproduces the page byte-for-
    # byte instead of leaving a blank line where the nav would go.
...
<body>
  <div class="page">
    {nav_block}<h1>{heading}</h1>
    <p class="subtitle">
    </p>

    <div class="section-label">Timeline</div>
    <div class="card">
      <h2>{timeline_card_heading}</h2>
      <p class="card-sub" style="margin: 0 0 10px 0;">{timeline_card_caption}</p>
    ...
    <div class="card">
      <h2>{map_card_heading}</h2>
      <p class="card-sub" style="margin: 0 0 10px 0;">{map_card_caption}</p>
      <p class="card-sub">
        {map_note}
      </p>
...
NAV_LINKS = [
    ("index.html", "Arthropods"),
    ("birds.html", "Birds"),
]


def build_nav_html(active: str) -> str:
    """Shared nav bar. `active` is the filename of the current page, matched
    against NAV_LINKS so it gets the highlighted style; every link is a
    relative path since this project site is served from a path prefix."""
